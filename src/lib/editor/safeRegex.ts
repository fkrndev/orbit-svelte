...
/**
 * A regex typed into the find bar is user input that runs on every keystroke,
 * against the whole document. A pattern like `(a+)+$` is valid and finite in
 * theory and hangs the webview in practice, so patterns are screened for
 * catastrophic backtracking before they are ever executed.
 *
 * Lifted from tolaria unchanged.
 */

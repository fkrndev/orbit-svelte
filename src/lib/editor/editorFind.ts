 ...
 *
 * Deliberately pure: matches are plain offsets into a string, and a replacement
 * is a `{from, to, insert}` triple. Nothing here knows about CodeMirror, so the
 * whole search model is testable without an editor — which matters, because the
 * regex path is the part that can silently eat a document.
 *
 * Lifted from tolaria's `utils/editorFind.ts`.
 */
...
export function findEditorMatches(
  ...
  while (match !== null) {
    // A zero-width match never advances `lastIndex`, so this would loop
    // forever. `^` and `\b*` are the patterns people actually type.
    if (match[0].length === 0) {
...
export function replacementForEditorFindMatch(
  ...
  // Re-running the pattern against the matched text alone is what makes `$1`
  // resolve to this match's capture groups rather than the first one's.

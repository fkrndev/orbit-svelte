...
export interface EdraCommand {
	...
	tooltip: string;
	/**
	 * A second line under the title, in the slash menu. Only worth spending on an
	 * item whose name does not already say what it does — "Bold" needs no help,
	 * "Link to file" does.
	 */
	description?: string;
	/**
	 * Extra words the slash menu matches on. People arrive from other editors
	 * typing the name *they* know, and a menu that returns nothing for that word
	 * is indistinguishable from a menu that cannot do it.
	 */
	aliases?: string[];
	shortCut?: string;

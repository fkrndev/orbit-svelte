...
<div
	...
	class="max-h-72 w-72 overflow-y-auto rounded-lg border border-border bg-popover p-1 text-popover-foreground shadow-md"
		...
		{#each grp.commands as command, commandIndex (commandIndex)}
			...
			>
				...
				<span class="flex min-w-0 flex-col items-start text-left">
						<span class="truncate">{command.tooltip}</span>
						{#if command.description}
							<span class="text-xs leading-snug text-muted-foreground">{command.description}</span>
						{/if}
					</span>
			</button>
		{/each}

...
export interface SlashCommandOptions {
	/**
	 * Groups contributed by the host app, listed after the built-in ones.
	 *
	 * The slash menu is where people look for "what can I put here", so an app
	 * feature that is otherwise only a keyboard shortcut belongs in it. It arrives
	 * as an option rather than being written into `GROUPS` because this extension
	 * is vendored: an item that reaches into the app's store would make the
	 * library un-liftable, and unusable from the headless round-trip test.
	 */
	extraGroups: SlashGroup[];
}
...
/**
 * The `/` menu's matcher, as a pure function so it can be tested without an
 * editor. Matching is on the title *and* the aliases; an empty query keeps
 * everything, which is what the menu shows the moment `/` is typed.
 */
export function filterSlashGroups(
	groups: SlashGroup[],
	query: string
): { name: string; title: string; commands: EdraCommand[] }[] {
	...
	return groups
		.map((group) => ({
			...
			commands: group.actions.filter((item) =>
				[item.tooltip, ...(item.aliases ?? [])].some((term) =>
					term.toLowerCase().trim().includes(needle)
				)
			)
		}))
...
export default (menuList: Component<any, any, ''>): Extension<SlashCommandOptions> =>
	Extension.create<SlashCommandOptions>({
		...
		onCreate() {
			...
			// Wide enough that a menu item's description reads as one or two lines
			...
			return [
				Suggestion({
					...
					items: ({ query }: { query: string }) =>
						filterSlashGroups([...GROUPS, ...options.extraGroups], query),
					render: () => {
						// eslint-disable-next-line @typescript-eslint/no-explicit-any
						let component: any;
